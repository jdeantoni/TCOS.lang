import type { FSMModel } from '../language/generated/ast.js';
import chalk from 'chalk';
import { Command } from 'commander';
import { FiniteStateMachineLanguageMetaData } from '../language/generated/module.js';
...
export const generateAction = async (fileName: string, opts: GenerateOptions): Promise<void> => {
    const services = createFsmServices(NodeFileSystem).Fsm;
...
function doGenerateCCFG(codeFile: CompositeGeneratorNode, model: FSMModel,debug:boolean): CCFG {
    const compilerFrontEnd = new TestFSMCompilerFrontEnd(debug);
    const ccfg = compilerFrontEnd.generateCCFG(model, true);
