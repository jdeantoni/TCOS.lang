...
import { FSMModel,FSM,Event,State,Transition } from "../../language-server/generated/ast.js";
...
// rule fsmInit
   ...
   //conclusion: initialState:[State:ID],starts:event
    ...
    createFSMLocalCCFG(node: FSM): CCFG {
        ...
        {
        let fsmInitStateModificationNode: Node = new Step(node, undefined, [new CreateVarInstruction(`${this.getASTNodeUID(node)}567`,`unknown`),new AssignVarInstruction(`${this.getASTNodeUID(node)}567`,`${node.initialState}`,`unknown`),new SetGlobalVarInstruction(`${this.getASTNodeUID(node)}currentState`,`${this.getASTNodeUID(node)}567`,`unknown`)])
        localCCFG.addNode(fsmInitStateModificationNode)
        {let e = localCCFG.addEdge(startsFSMNode,fsmInitStateModificationNode)
        e.guards = [...e.guards, ...[]]}
        startsFSMNode = fsmInitStateModificationNode
        }
    
        startsFSMNode.params = [...startsFSMNode.params, ...[]]
        startsFSMNode.returnType = "void"
        startsFSMNode.functionsNames = [`${startsFSMNode.uid}fsmInit`] //overwrite existing name
   ...
   //conclusion: terminates:event
    ...
    createEventLocalCCFG(node: Event): CCFG {
        ...
        startsEventNode.functionsDefs =[...startsEventNode.functionsDefs, ...[]] //GG
                //mark 1 { "name": "terminates", "type": "event"}
        {let e = localCCFG.addEdge(startsEventNode,terminatesEventNode)
        e.guards = [...e.guards, ...[]]}
...
// rule stateEnd
   ...
   //conclusion: terminates:event
    ...
    createStateLocalCCFG(node: State): CCFG {
        ...
        startsStateNode.returnType = "void"
        startsStateNode.functionsNames = [`${startsStateNode.uid}stateInit`] //overwrite existing name
        ...
        outTransitionsHole.functionsNames = [`${outTransitionsHole.uid}stateEnd`] //overwrite existing name
...
// rule transitionEnd
   ...
   //conclusion: terminates:event
    ...
    createTransitionLocalCCFG(node: Transition): CCFG {
        let localCCFG = new CCFG()
        let startsTransitionNode: Node = new Step(node,NodeType.starts,[new CreateGlobalVarInstruction(`${this.getASTNodeUID(node)}isSensitive`,`bool`), new SetGlobalVarInstruction(`${this.getASTNodeUID(node)}isSensitive`,`false`,`bool`)])
        ...
        let sentEventHole: Hole = new Hole(node.sentEvent.ref)
        localCCFG.addNode(sentEventHole)
        ...
        startsTransitionNode.returnType = "void"
        startsTransitionNode.functionsNames = [`${startsTransitionNode.uid}transitionInit`] //overwrite existing name
        startsTransitionNode.functionsDefs =[...startsTransitionNode.functionsDefs, ...[]] //GG
    
        guardEventHole.params = [...guardEventHole.params, ...[]]
        guardEventHole.returnType = "void"
        guardEventHole.functionsNames = [`${guardEventHole.uid}fire`] //overwrite existing name
        guardEventHole.functionsDefs =[...guardEventHole.functionsDefs, ...[]] //GG
                //mark 0
        {let e = localCCFG.addEdge(guardEventHole,sentEventHole)
        e.guards = [...e.guards, ...[]]}
            
        sentEventHole.params = [...sentEventHole.params, ...[]]
        sentEventHole.returnType = "void"
        sentEventHole.functionsNames = [`${sentEventHole.uid}transitionEnd`] //overwrite existing name
        sentEventHole.functionsDefs =[...sentEventHole.functionsDefs, ...[]] //GG
    
                //conclusion participants in sequential collection but not a hole: { "name": "terminates", "type": "event"}
                                    //mark 2
        localCCFG.addEdge(sentEventHole,targetHole)
                    sentEventHole = targetHole
