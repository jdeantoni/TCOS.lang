...
void function11initializeVar()
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		*((int *)sigma["Variable2_0_2_11currentValue"]) = Variable2_0_2_111387;
	}
}
int function18accessVarRef()
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		VarRef4_7_4_91593 = *(int *)sigma["Variable0_0_0_10currentValue"];
	}
...
void function26executeAssignment2(int resRight)
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		*((int *)sigma["Variable0_0_0_10currentValue"]) = Assignment6_4_6_112534;
	}
}
void function32executeAssignment2(int resRight)
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		*((int *)sigma["Variable1_0_1_10currentValue"]) = Assignment7_4_7_112534;
	}
}
int function27accessVarRef()
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		VarRef6_9_6_111593 = *(int *)sigma["Variable1_0_1_10currentValue"];
	}
...
int function33accessVarRef()
{
	...
	{
		const std::lock_guard<std::mutex> lock(sigma_mutex);
		VarRef7_9_7_111593 = *(int *)sigma["Variable2_0_2_11currentValue"];
	}
