...
const { CCFGInterpreter } = await import(pathToFileURL(path.join(repoRoot, "packages/interpreterCCFG/out/src/InterpretCCFG.js")).href);
...
if (process.env.TRACE === "1") {
    for (let i = 0; i < 80; i++) {
        ...
        if (snapshot.status === "terminated" || snapshot.status === "error" || snapshot.status === "stopped") {
            break;
        }
    }
} else {
    result = await interpreter.run({ maxSteps: 10_000 });
}
